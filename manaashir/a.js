...
;(function(){
	...
	focus_first_element = rakkazawwal = function (e) {
		if (e) {
			var s = e.querySelector('input')
				...
				|| e.querySelector('[contenteditable]');
			if (s) {
				s.focus();
				webapp.scrollto(s);
				return s;
...
;(function(){
	var settingsitems = [
		...
		['reportbug', 0, function () {
			activity.abrad(myemail+'?subject='+appname+' bug '+224);
		}],
		['requestfeat', 0, function () {
			activity.abrad(myemail+'?subject='+appname+' request '+224);
		}],
		['timeformat', function () {
	...
	softkeys = {
		...
		* */
		...
		press: function (k, e, longpress) {
			...
			|| a.contentEditable == 'true') && a.type != 'range') {
				editmode = 1,
			...
			* */
			if (editmode) {
				...
				if ( atstart && (k == K.up) && !e.altKey ) caught = focusprev(a), pd();
				if ( atend && (k == K.dn) && !e.altKey ) caught = focusnext(a), pd();
				else if (k == K.en && e.shiftKey && a.uponshiftenter) a.uponshiftenter(atstart, atend), pd();
			...
			* */
			...
			&& [K.up, K.dn, K.lf, K.rt, K.en].includes(k)) {
				...
				if (selectionorigin !== undefined) {
					...
					else if (k == K.rt && a.selectionEnd < length)
						++a.selectionEnd, pd();
...
;(function(){
		...
		save: function () {
			...
			for (var i in c) {
				if (c.hasOwnProperty(i)) {
					...
					if (o.type == 0) {
						save_array.push( [0, o.place, k.text.value] );
					}
					if (o.type == 1) {
	...
	Hooks.set('ready', function (args) {
		...
		editorlist = list( mfateeh.list ).idprefix( 'editorlist' )
					.listitem('scene');
		...
		editorlist.after_set = function (o, c, k) {
			k.text.onprev = function () {
				editorlist.up();
			};
			k.text.onnext = function () {
				editorlist.down();
			};
